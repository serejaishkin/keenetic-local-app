...
class RouterViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * Из ROADMAP.md ("Ещё не закрыто"): индикатор "нет подключения к
     * роутеру" отдельно от общего текста ошибки. Обновляется в
     * loadInterfaces() - она уже дёргается каждые 5 сек с Dashboard, так
     * что это состояние актуализируется само по себе без доп. опроса.
     */
    private val _routerUnreachable = MutableStateFlow(false)
    val routerUnreachable: StateFlow<Boolean> = _routerUnreachable.asStateFlow()
    ...
    /**
     * Скан эфира. ПОДТВЕРЖДЕНО HAR (06.08) - читается батч-POST на /rci/,
     * НЕ простым GET (в этом отличие от большинства других show-эндпоинтов
     * в проекте). Ответ роутера - массив из одного элемента
     * [{"show":{"site-survey":{"ap_cell":[...]}}}] - распаковываем сразу до
     * "site-survey", чтобы карточка показывала список сетей, а не три слоя
     * обёртки show/site-survey вокруг него.
     */
    ...
    /**
     * ПОДТВЕРЖДЕНО HAR (06.08) - и чтение, и запись реальные, с успешным
     * ответом роутера ("components auto-update enabled."). disable=false
     * значит автообновление ВКЛЮЧЕНО (инвертированное поле - осторожно).
     */
            ...
            try {
                ...
                if (response.isSuccessful) {
                    val enabled = runCatching {
                        response.body()?.asJsonArray?.get(0)?.asJsonObject
                            ?.getAsJsonObject("show")?.getAsJsonObject("sc")
                            ?.getAsJsonObject("components")?.get("auto-update")?.asBoolean
                    }.getOrNull()
                    _autoUpdateEnabled.value = enabled
                }
            } catch (e: Exception) {
                AppLogger.logAction("Auto-update status load failed", e.message ?: "")
            }
        }
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    _error.value = "Ошибка изменения автообновления: HTTP ${response.code()}"
                }
            } catch (e: Exception) {
                _error.value = "Ошибка изменения автообновления: ${e.message}"
            }
        }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    val body = response.body()
                    val extracted = runCatching {
                        body?.asJsonArray?.get(0)?.asJsonObject
                            ?.getAsJsonObject("show")
                            ?.get("site-survey")
                    }.getOrNull()
                    _siteSurveyRaw.value = if (extracted != null) {
                        com.keenetic.local.ui.screens.common.ApiCallState.Success(extracted)
                    } else {
                        com.keenetic.local.ui.screens.common.ApiCallState.Error("неожиданный формат ответа")
                    }
                } else {
                    _siteSurveyRaw.value = com.keenetic.local.ui.screens.common.ApiCallState.Error("HTTP ${response.code()}")
                }
            } catch (e: Exception) {
                _siteSurveyRaw.value = com.keenetic.local.ui.screens.common.ApiCallState.Error(e.message ?: "неизвестная ошибка")
                AppLogger.logAction("Site survey scan failed", e.message ?: "")
            }
        }
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    _routerUnreachable.value = false
                } else {
                    _error.value = "HTTP ${response.code()}"
                    _routerUnreachable.value = false
                }
            } catch (e: java.io.IOException) {
                // Раньше (ROADMAP.md, "Ещё не закрыто") это падало в общий
                // _error - неотличимо от обычной ошибки на конкретном
                // экране. IOException конкретно значит "не достучались до
                // роутера по сети" (таймаут/connection refused/no route),
                // в отличие от HTTP-ошибки, где роутер ответил, но с кодом
                // ошибки. Раз это дёргается каждые 5 сек с Dashboard - самое
                // надёжное место поймать реальную потерю связи с роутером.
                _routerUnreachable.value = true
                AppLogger.logAction("Router unreachable", e.message ?: "")
            } catch (e: Exception) {
    ...
    /**
     * ПОДТВЕРЖДЕНО HAR (06.08): формат ниже - точное совпадение с реальным
     * успешным запросом создания расписания с веб-морды (dow - 0=Вс..6=Сб,
     * та же индексация, что уже используется в dayNames в ScheduleCard).
     * Ответ роутера на реальный вызов: {"schedule":{"status":[{"status":
     * "message",...,"message":"created schedule \"...\""}]}}.
     */
    ...
    /**
     * ПОДТВЕРЖДЕНО HAR (06.08, реальный успешный вызов на роутере, ответ
     * "rule accepted."). Формат оказался СОВСЕМ не таким, как в первой
     * версии этой функции (та была угадана по аналогии с ip.static и
     * НИКОГДА не была подтверждена - оставляю этот факт в истории коммитов
     * как урок: угаданный формат лучше не коммитить как рабочий).
     *
     * Реальная структура:
     * 1) "access-list" - массив с ОДНИМ элементом {acl, permit:{...поля}}.
     *    "acl" - имя списка вида "_WEBADMIN_<имя WAN-интерфейса>" (именно
     *    так его генерирует сама веб-морда для правил, создаваемых через
     *    UI, а не через CLI напрямую).
     * 2) "interface" - привязка ACL к интерфейсу через access-group/direction.
     *    Без этого шага правило создаётся, но не применяется к трафику.
     * 3) "system.configuration.save" - как обычно.
     *
     * ОГОВОРКА: в HAR подтверждён только action="permit". Для "deny" ключ
     * "permit" в JSON, вероятно, меняется на "deny" (так же, как в CLI
     * `ip access-list <name> permit|deny`), но HAR-подтверждения именно
    ...
    fun createFirewallRule(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val aclName = "_WEBADMIN_$wanId"
                val verbKey = if (action == "deny") "deny" else "permit" // "permit" - подтверждено HAR, "deny" - экстраполяция

                val ruleFields = mutableMapOf<String, Any>(
                    "index" to 0,
                    "action" to action,
                    "source" to sourceIp,
                    "source-mask" to sourceMask,
                    "destination" to destIp,
                    "destination-mask" to destMask,
                    "disable" to false,
                    "schedule" to mapOf("no" to true)
                )
                if (protocol.isNotBlank()) ruleFields["protocol"] = protocol
